...
type Configuration struct {
	...
	// IngressIDs contains all the ingress id
...
const (
	Active GrayType = iota
	ActiveGray
	InactiveGray
)

type IngGray struct {
	Type GrayType `json:"grayType"`
}
...
type Server struct {
	...
	SSLPassthrough bool `json:"sslPassthrough"`
	// SSLCerts describes the certificates that will be used on the server
	...
	// NeedDefaultCert indicates whether the server requires a default cert
...
type Location struct {
	...
	// LocationPreceding indicates preceding for location
	// By default this is ""
	LocationPreceding string `json:"location-preceding"`
	// LocationPathPrefix indicates prefix for location path
	// By default this is ""
	LocationPathPrefix string `json:"location-path-prefix"`
	// LocationPathEscape indicates is need escape location path
	// By default this is ""
	LocationPathEscape bool `json:"location-path-escape"`
	// DisableRobots allows to disable the robots
...
// Secret holds the definition of an Secret plus its annotations
type Secret struct {
	apiv1.Secret      `json:"-"`
	SSLCert           *SSLCert               `json:"sslCert"`
	ParsedAnnotations *secannotations.Secret `json:"parsedAnnotations"`
}
...
// Canary describes upstream and rule of the canary ingress
type Canary struct {
	// BackendTarget indicates which target key relation backend service for this location on backend-target mode
	// By default this is nil
	Target string `json:"backend-target"`
	// Policies to describe the characteristics of an alternative backend.
	// +optional
	TrafficShapingPolicy TrafficShapingPolicy `json:"trafficShapingPolicy"`
}

// ChecksumStatus contains status of ingress and secret
type ChecksumStatus struct {
	IngChecksumStatus    bool
	SecretChecksumStatus bool
}
