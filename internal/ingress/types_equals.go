...
func (s1 *Server) Equal(s2 *Server) bool {
	...
	if len(s1.SSLCerts) != len(s2.SSLCerts) {
		return false
	}
	for idx, s1s := range s1.SSLCerts {
		if !s1s.Equal(s2.SSLCerts[idx]) {
			return false
		}
	}
	...
	if s1.NeedDefaultCert != s2.NeedDefaultCert {
		return false
	}
...
func (l1 *Location) Equal(l2 *Location) bool {
	...
	if l1.LocationPreceding != l2.LocationPreceding {
		return false
	}

	if l1.LocationPathPrefix != l2.LocationPathPrefix {
		return false
	}

	if l1.LocationPathEscape != l2.LocationPathEscape {
		return false
	}
