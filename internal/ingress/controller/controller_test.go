...
func TestMergeAlternativeBackends(t *testing.T) {
	...
	for title, tc := range testCases {
		t.Run(title, func(t *testing.T) {
			nginx.mergeAlternativeBackends(tc.ingress, tc.upstreams, tc.servers)
...
func TestExtractTLSSecretName(t *testing.T) {
	...
	for title, tc := range testCases {
		t.Run(title, func(t *testing.T) {
			names := extractTLSSecretName(tc.host, tc.ingress, tc.fn)
			if len(names) > 0 {
				...
				t.Errorf("Expected Secret name %q (got %v)", tc.expName, names)
			}
		})
	}
}
