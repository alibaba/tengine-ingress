...
func (s *k8sStore) syncSecret(key string, mc metric.Collector) {
	s.syncSecretMu.Lock()
	defer s.syncSecretMu.Unlock()

	klog.Infof("Syncing Secret %q", key)
	cert, err := s.getPemCertificate(key)
	if err != nil {
		if !isErrSecretForAuth(err) {
			klog.Errorf("Error obtaining X.509 certificate [%q]: %v", key, err)
			mc.IncSSLCertVerifyFailCount()
		}
		return
	}
