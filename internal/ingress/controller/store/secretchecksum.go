...
// SecretCheckSumLister makes a Store that lists SecretCheckSum
type SecretCheckSumLister struct {
	cache.Store
}

// ByKey returns the SecretCheckSum matching key in the local store or an error
func (il SecretCheckSumLister) ByKey(key string) (*secretcheckv1.SecretCheckSum, error) {
	...
	return i.(*secretcheckv1.SecretCheckSum), nil
}
