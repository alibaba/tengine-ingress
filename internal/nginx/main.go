...
// Version return details about Tengine
func Version() string {
	flag := "-v"
	...
	cmd := exec.Command("tengine", flag)
	out, err := cmd.CombinedOutput()
	if err != nil {
		klog.Errorf("unexpected error obtaining Tengine version: %v", err)
		return "N/A"
	}
...
// IsRunning returns true if a process with the name 'tengine' is found
func IsRunning() bool {
	processes, _ := ps.Processes()
	for _, p := range processes {
		if p.Executable() == "tengine" {
			return true
		}
	}
...
func WorkerProcessesNum() int {
	processesNum := 0
	processes, _ := ps.Processes()
	for _, p := range processes {
		klog.Infof("Tengine process [%v]", p.Executable())
		if p.Executable() == "tengine" {
			processesNum++
		}
	}
